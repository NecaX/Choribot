...
function setup(dcBot, tgBot, dcUsers, messageMap, bridgeMap, settings) {
	...
	dcBot.on("message", (message) => {
		...
		if (message.author.id === dcBot.user.id) {
			var D2TCommand=message.content;
			var usermessage="";
			while (D2TCommand.slice(0,1) != ":" && D2TCommand !== ""){ //Se elimina el nombre de la persona que ha emitido el mensaje
				usermessage = usermessage.concat(D2TCommand.slice(0,1));
				D2TCommand = D2TCommand.slice(1);
			}
			if (D2TCommand == "") return;
			usermessage = usermessage.slice(2, -2); //Eliminamos * para que quede el nombre de usuario del que escribió el comando
			if (typeof dcUsers.lookupUsername(usermessage) !== "undefined") {
				return;
			}
		}
		...
		if (bridge !== undefined) {
			...
			/*message.reply(
				"Este canal de Discord no envia mensajes a Telegram"
			);*/
		}
	});
